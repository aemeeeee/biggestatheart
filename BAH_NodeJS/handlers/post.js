...
const viewAllPosts = async (request, response) => {
  try{
    db.clientPool.query('SELECT * FROM posts INNER JOIN users ON posts.authorid = users.userid ORDER BY isflagged desc, sightingtime desc ', (error, result) => {
      if (error) {
...
const viewAllVerifiedPosts = async (request, response) => {
  try{
    db.clientPool.query('SELECT * FROM posts INNER JOIN users ON posts.authorid = users.userid WHERE verified ORDER BY isflagged desc, sightingtime desc', (error, result) => {
      if (error) {
        response.send(error.message)
      }
      else if (result.rowCount != 0) {
...
const viewAllUnverifiedPosts = async (request, response) => {
  try{
    db.clientPool.query('SELECT * FROM posts INNER JOIN users ON posts.authorid = users.userid WHERE NOT verified ORDER BY isflagged desc, sightingtime desc', (error, result) => {
      if (error) {
        response.send(error.message)
      }
      else if (result.rowCount != 0) {
...
const viewUserPosts = async (request, response) => {
  ...
  try {
    ...
    db.clientPool.query('SELECT * FROM posts INNER JOIN users ON posts.authorid = users.userid WHERE userid = $1 ORDER BY isflagged desc, sightingtime desc', [userid], (error, result) => {
      if (error) {
        response.send(error.message)
      }
      else if (result.rowCount != 0) {
